...
equations courtesy of \cite{RSCK}, where $\oa{h}$ denotes the orbital
momentum vector and $\oa{e}$ the eccentricity vector.

\begin{align}
  \oa{h} &:= \Pos \times \Vel\nonumber\\
  a &:= \left(2\cdot\absvec{\Pos}^{-1} - \frac{2}{\nu}\cdot\absvec{\Pos} \right)^{-1}\\
  \oa{e} &:= \frac{\Vel\times \oa{h}}{\nu} - \absvec{\Pos}\nonumber\\
  e &:= \absvec{\oa{e}}\\
