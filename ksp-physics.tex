...
\title{Documentation of KSP Physics\\\small{Version \input{.tagversion}}\\\vspace{1 em}\url{https://github.com/mhoram-kerbin/ksp-physics-documentation}}
...
\index{Drag} Drag is the force applied to rocket based on the movement through the
...
and thrust $\F{T}$.  The \index{Acceleration} acceleration vector
$\oa{a}$ based on these three forces is:
...
\subsubsection{True Anomaly to Eccentric Anomaly} \label{ta2ea}
...
\subsubsection{Mean Anomaly to Eccentric Anomaly} \label{ma2ea}
...
\subsubsection{Eccentric Anomaly to True Anomaly} \label{ea2ta}
...
\subsubsection{Eccentric Anomaly to Mean Anomaly} \label{ea2ma}
...
\subsubsection{True Anomaly to Mean Anomaly}

Use the methods in sections \ref{ta2ea} and \ref{ea2ma}.

\subsubsection{Mean Anomaly to True Anomaly}
...
We can calculate the \index{Apoapsis} apoapsis $Apo$ and
\index{Periapsis} periapsis $Peri$ as
