...
the burn $M$, Specific impulse $I_{SP}$ \footnote{Remember that the
  ...
  pressure.} and the thrust $\absvec{\oa{F_T}}$ it is possible to
...
burn, we use the Tsiolkovsky rocket equation \index{Tsiolkovsky Rocket Equation}
...
\begin{align}
  T_B & = \frac{M_F}{\dot{M}}\nonumber\\
  & = \frac{M}{\absvec{\oa{F_T}}}\cdot I_{SP} \cdot g_0\left(1-\exp\left(-\frac{\Delta V}{I_{SP}\cdot g_0}\right)\right)
\end{align}

Note however that this is only valid if the current stage in the
rocket has enough $\Delta V$ for the complete burn.

\section{Thrust Weight Ratio}

The Thrust Weight ratio \index{Thrust Weight Ratio} is a simple yet
complex measurement.
