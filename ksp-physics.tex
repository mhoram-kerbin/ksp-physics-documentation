...
\title{Documentation of Pre V1.0 KSP Physics\\\small{Version \input{.tagversion}}\\\vspace{1 em}\url{https://github.com/mhoram-kerbin/ksp-physics-documentation}\\\url{http://forum.kerbalspaceprogram.com/threads/93426-Physics-of-KSP}}
...
\begin{abstract}
  ...
  This document summarizes the Pre V1.0 stock physics of Kerbal Space
  Program \cite{KSP}. It should be valid for versions 0.21 to
  0.25.0. Some equations are also applicable to real life situations -
  if you ever happen to stumble into one.

  Aspects of Stock Aerodynamic have changed in KSP V 1.0 So these
  sections do not apply anymore.
...
\begin{align}
  ...
  &= - 0.5 \cdot \rho(alt)\cdot d \cdot 0.008\frac{m^2}{kg} \cdot M \cdot \absvec{\oa{SV}} \cdot \oa{SV}\label{DragForceFormula}
\end{align}

\section{Gravity}\label{GravityForce}
...
\begin{align}
  ...
  \F{G} &:= - M \cdot \mu^{(Name)}\cdot\frac{\Pos}{\absvec{\Pos}^3}\label{GravityForceFormula}
\end{align}
...
\chapter{Orbital Transfers}

\section{Hohmann Transfer}

\subsection{Delta-V Calculations}

\subsection{Phase Angles}

\section{Bi-elliptic Transfer}

\section{Ideal Transfer Method}
...
\begin{align}
  ...
  \dv_G & :\approx \sqrt{G\cdot PMass^{(Name)}\left(\frac{1}{PRadius^{(Name)}}-\frac{1}{r_o}\right)}\\
...
The terminal velocity \cite{TerminalVelocity} \index{Terminal
  Velocity} $V_t$ of an object in vertical motion is the velocity at
which the sum of drag force and buoyancy \cite{Buoyancy} equals the
gravity force. Since buoyancy is not modeled in KSP, it is left out in
the following calculation.

As described in equations \eqref{DragForceFormula} and
\eqref{GravityForceFormula} the forces are

\begin{align*}
   \F{D} &= - 0.5 \cdot \rho(alt)\cdot d \cdot 0.008\frac{m^2}{kg} \cdot M \cdot \absvec{\oa{SV}} \cdot \oa{SV}\\
   \F{G} &= - M \cdot \mu^{(Name)}\cdot\frac{\Pos}{\absvec{\Pos}^3}
\end{align*}

Setting them equal results in the terminal velocity for an object with
drag coefficient $d$ at altitude $alt$ of

\begin{align}
   0.5 \cdot \rho(alt)\cdot d \cdot 0.008\frac{m^2}{kg} \cdot M \cdot \absvec{\oa{SV}}^2 \cdot \frac{\oa{SV}}{\absvec{\oa{SV}}} & = M \cdot \mu^{(Name)}\cdot\frac{\Pos}{\absvec{\Pos}^3}\nonumber\\
   0.5 \cdot \rho(alt)\cdot d \cdot 0.008\frac{m^2}{kg} \cdot \absvec{\oa{SV}}^2 & = \mu^{(Name)}\cdot\frac{1}{\absvec{alt + PRadius^{(Name)}}^2}\nonumber\\
    \absvec{\oa{SV}} & = \frac{\sqrt{\frac{250 \frac{kg}{m^2} \cdot\mu^{(Name)}}{\rho(alt)\cdot d}}}{\absvec{alt + PRadius^{(Name)}}}\nonumber\\
   V_t(alt, d) & := \frac{\sqrt{\frac{250 \frac{kg}{m^2} \cdot\mu^{(Name)}}{\rho(alt)\cdot d}}}{\absvec{alt + PRadius^{(Name)}}}
\end{align}

\subsection{Kerbin}
For Kerbin the Terminal velocities according to this formula are like
on the Wikipage \cite{Kerbin}.

\begin{tabular}{rr}
  Altitude (m) & Terminal Velocity (m/s)\\
  75 & \\
  1000 & \\
\end{tabular}
...
Also Buoyancy \cite{Buoyancy} \index{Buoyancy} is not modeled in KSP,
which describes the force that an object receives by displacing a
liquid or air.
...
\bibitem{AscentDeltaVApproximationFormula}
  \url{http://forum.kerbalspaceprogram.com/threads/97801-I-need-help-making-a-delta-v-map?p=1498127#post1498127}
...
\bibitem{TerminalVelocity}
  \url{http://en.wikipedia.org/wiki/Terminal_velocity}

\bibitem{Buoyancy}
  \url{http://en.wikipedia.org/wiki/Buoyancy}

%\bibitem{KerbinTerminalVelocity}
%  \url{}
